...
def obtener_respuesta(user_input):
    ...
    instructions = (
        "Eres un asesor emocional, especializado en brindar apoyo y orientación en situaciones emocionales. "
        "Tu objetivo es ofrecer respuestas empáticas y alentadoras para ayudar a los usuarios a manejar sus emociones de manera positiva."
    )
...
def guardar_conversacion(conversation):
    ...
    with open("conversacion_emocional.json", "w") as file:
...
def conversar_con_voz():
    ...
    ohbot.say("Soy Versa tu asistente Emocional en que te puedo ayudar")
    while True:
