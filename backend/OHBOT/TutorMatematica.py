...
def obtener_respuesta(user_input):
    ...
    instructions = (
        "Eres un tutor de matemáticas, especializado en ayudar con problemas matemáticos y explicar conceptos. "
        "Ofreces respuestas claras y concisas para ayudar a los estudiantes a comprender y resolver problemas matemáticos."
    )
...
def guardar_conversacion(conversation):
    ...
    with open("conversacion_matematicas.json", "w") as file:
...
def conversar_con_voz():
    ...
    ohbot.say("Soy Versa tu Tutor de matematica en que te puedo ayudar")
    while True:
